...
1. **Verify Mon 27 Apr 07:00 BST WebBank Sync fire** (post re-cron) — first natural fire of the new parser + the toggle-cycle 403 fix attempt. Expected: 94 rows parsed, ~0 changes vs 26 Apr snapshot, **`git push origin main` succeeds**, Slack digest posts, Mirror DB unchanged. If push still 403s: delete + recreate the trigger with `allow_unrestricted_git_push` set at creation time (not via post-create update). Working hypothesis: credential provisioned at create-time without push scope; post-hoc toggle changes don't refresh it.
2. **Verify Mon 27 Apr 07:30 BST Daily Brief fire** (post re-cron) — first natural fire. Should reference today's freshly-pushed WebBank diff and write `snapshots/daily_brief_<today>.json` (new artefact step added 2026-04-26). Confirm Slack post lands cleanly + JSON artefact pushes to `main`.
3. **Verify Mon 27 Apr 08:30 BST Weekly Monday fire** (new routine) — first natural fire of `weekly_monday`. Should read the Daily Brief artefact and produce a draft week-ahead. First-cycle behaviour expected: `had_week_ahead_plan = false`, no carry-over from prior Friday.
4. **Manual UI work in routines panel** (do before Mon 27 Apr fires):
   - **Re-cron Daily Brief**: `30 6 * * 1-5` (07:30 BST). Routine `trig_0168M4zWyESpqBw57f9PDXfp`.
   - **Re-cron WebBank Sync**: `0 6 * * 1-5` (07:00 BST). Routine `trig_01V1a8NQ1SayN7kpnq1Q7Eh2`.
   - **Create Weekly Monday routine** — cron `30 7 * * 1` (08:30 BST), model `claude-sonnet-4-6`, MCPs: Slack + Notion + Google-Calendar + Gmail, repo source `YagoReeves/flex-card` with `allow_unrestricted_git_push: true` set at creation. Wrapper reads `prompts/weekly_monday.md`.
   - **Create Weekly Friday routine** — cron `0 16 * * 5` (17:00 BST), same MCP/repo setup. Wrapper reads `prompts/weekly_friday.md`.
...
6. **End-to-end workflow verification** after Jobs 2 + 4 are live. Run a full week (or two), capture friction, missed signals, false positives.
...
- [~] Build **Weekly Cycle** — prompts written 2026-04-26 (`prompts/weekly_monday.md`, `prompts/weekly_friday.md`); routines pending creation in UI. Crons: Mon `30 7 * * 1` (08:30 BST), Fri `0 16 * * 5` (17:00 BST). Daily Brief now writes `snapshots/daily_brief_<date>.json` so Weekly Monday can read it as primary input. Friday writes `snapshots/weekly_friday_<date>.json` with `commitments_for_next_week` for Mon's continuity loop. Audience framing: drafts in `#flex-agent-jago` only for v1 (Jago manually publishes onward to `#product-cleo-card` post-edit).
